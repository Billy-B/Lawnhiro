...
namespace BB
{
    internal class DatabaseRepository : IObjectRepository
    {
                    ...
                    using (IDbCommand command = conn.CreateCommand())
                    {
                        ...
                        if (objectsToInsert != null)
                        {
                            foreach (var grouping in objectsToInsert)
                            {
                                ...
                                {
                                    QueryMetadata outputMetadata = QueryMetadata.SimpleColumnMetadata(manager.InsertOutputColumns);
                                    QueryNode node = outputMetadata.FirstNode;
                                    foreach (object obj in grouping)
                                    {
                                        ...
                                        {
                                            reader.Read();
                                            extender.DataSource = new DatabaseDataRow(node, reader);
                                        }
        ...
        public IEnumerable<DatabaseDataRow> EnumerateRows(QueryMetadata metadata)
        {
            DatabaseManagement.SQL.SelectStatement selectStatement = DatabaseManagement.SQL.Statement.SelectFrom(metadata.TableExpression, metadata.ColumnExpressions);
            QueryNode firstNode = metadata.FirstNode;
            using (IDbConnection connection = Database.GetConnection())
            {
                ...
                using (IDbCommand command = connection.CreateCommand())
                {
                    ...
                    using (IDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            yield return new DatabaseDataRow(firstNode, reader);
                        }
                    }
                }
            }
        ...
        public IEnumerable<DatabaseDataRow> EnumerateRows(QueryMetadata metadata, ConditionalExpression whereExpression)
        {
            DatabaseManagement.SQL.SelectStatement selectStatement = DatabaseManagement.SQL.Statement.SelectFrom(metadata.TableExpression, metadata.ColumnExpressions, whereExpression);
            QueryNode firstNode = metadata.FirstNode;
            using (IDbConnection connection = Database.GetConnection())
            {
                connection.Open();
